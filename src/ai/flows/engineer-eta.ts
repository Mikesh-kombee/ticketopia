...
const EngineerEtaInputSchema = z.object({
  engineerLocation: z
    .string()
    .describe('The current location of the engineer as a string (e.g., latitude, longitude).'),
  ticketLocation: z
    .string()
    .describe('The location of the ticket as a string (e.g., latitude, longitude).'),
...
const EngineerEtaOutputSchema = z.object({
  etaMinutes: z
    .number()
    .describe('The estimated time of arrival in minutes, based on current traffic conditions.'),
  explanation: z
    .string()
    .describe('A brief explanation of how the ETA was calculated, considering distance and traffic.'),
});
export type EngineerEtaOutput = z.infer<typeof EngineerEtaOutputSchema>;

export async function engineerEtaCalculation(input: EngineerEtaInput): Promise<EngineerEtaOutput> {
