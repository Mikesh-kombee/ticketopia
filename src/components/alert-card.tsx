...
const severityConfig: Record<
  AlertSeverity,
  { color: string; icon: React.ElementType; borderColor: string }
> = {
  high: {
    color: "text-red-500",
    icon: AlertTriangle,
  ...
  return (
    ...
    >
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            />
            <SeverityIcon
              className={cn("h-6 w-6", severityConfig[alert.severity].color)}
            />
            <div>
      ...
      <Accordion
        type="single"
        collapsible
        value={isAccordionOpen ? alert.id : ""}
        ...
        <AccordionItem value={alert.id} className="border-b-0">
          ...
          <AccordionContent className="px-6 pb-4 pt-0">
            <div className="space-y-4">
              ...
              <div
                ref={mapRef}
                className={cn("h-48 w-full rounded-md bg-muted", {
                  hidden: !isMapsApiLoaded || mapsApiError,
                })}
              ></div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(value) =>
                      onNotificationChange(alert.id, "push", value)
                    }
                  />
                </div>
                <div className="flex items-center justify-between">
                  <Label
                    htmlFor={`email-${alert.id}`}
                    className="flex items-center gap-2 text-sm"
                  >
                  ...
                  <Switch
                    ...
                    onCheckedChange={(value) =>
                      onNotificationChange(alert.id, "email", value)
                    }
                  />
                </div>
              </div>
