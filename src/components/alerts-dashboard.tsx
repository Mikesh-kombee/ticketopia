...
const generateMockAlert = (idSuffix: number): Alert => {
  ...
  let details = `Investigate ${randomType.toLowerCase()} for ${
    randomEngineer.name
  }.`;
  if (randomType === "Speeding")
    details = `${randomEngineer.name} detected speeding at ${Math.floor(
      Math.random() * 30 + 80
    )} km/h.`;
  if (randomType === "Long Idle")
    details = `${randomEngineer.name} has been idle for ${Math.floor(
      Math.random() * 30 + 10
    )} minutes.`;
  ...
  return {
    ...
    timestamp: new Date(
      Date.now() - Math.random() * 1000 * 60 * 60 * 5
    ).toISOString(), // within last 5 hours
    type: randomType,
    severity: randomSeverity,
    engineerId: randomEngineer.id,
    engineerName: randomEngineer.name,
    location,
    locationSnippet: `Near ${location.lat.toFixed(3)}, ${location.lng.toFixed(
      3
    )}`,
    details,
    routeTrace:
...
export function AlertsDashboard() {
  ...
  const handleNotificationChange = (
    ...
    setAlerts((prev) =>
      prev.map((a) =>
        a.id === alertId
          ? { ...a, notifications: { ...a.notifications, [type]: value } }
          : a
      )
    );
  };
  ...
  const handleBulkAction = (status: AlertStatus) => {
    setAlerts((prev) =>
      prev.map((a) => (selectedAlerts.has(a.id) ? { ...a, status } : a))
    );
    setSelectedAlerts(new Set());
  };
  ...
  const isIndeterminate =
    selectedAlerts.size > 0 && selectedAlerts.size < filteredAlerts.length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end p-4 bg-card rounded-lg shadow">
        ...
        <Select
          value={filterType}
          onValueChange={(value) => setFilterType(value as AlertType | "all")}
        >
        ...
        <Select
          value={filterStatus}
          onValueChange={(value) =>
            setFilterStatus(value as AlertStatus | "all")
          }
        >
          <SelectTrigger className="w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 p-4 bg-card rounded-lg shadow">
        <div className="flex items-center space-x-3">
          <Checkbox
            ...
            className={
              isIndeterminate
                ? "[&[data-state=checked]>span]:bg-muted-foreground"
                : ""
            }
          />
      ...
      <ScrollArea className="h-[calc(100vh-480px)] md:h-[calc(100vh-420px)] pr-3">
        {" "}
        {/* Adjusted height based on typical viewport */}
